...
export function validateDataPartnerRegister(req, res, next) {
  const { businessName, email, password, location, type } = req.body;
  if (!email || !password || !location || !type || !businessName) {
    return next(
      new ResponseError(
        400,
        "Missing required fields: business name, email, password, location, type"
      )
    );
  }
  if (
    !isValidInput(businessName) ||
    !isValidInput(email) ||
...
export function validateDataPartnerLogin(req, res, next) {
